...
function Refrigerators({ limit, extraCard, maxPrice, isFilterActive }) {
    const navigate = useNavigate();
    const [allRefrigerators, setAllRefrigerators] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        fetch("https://e-commerce-3-zvas.onrender.com/user/get/refrigerator", {
            method : "GET",
            headers : { "Content-type" : "application/json" }
        })
          .then((res) => res.json())
          .then((data) => {
            setAllRefrigerators(data.allRefrigerators);
            setTimeout(() => setLoading(false), 1000);
          });
    }, []);

    const displayedRefrigerators = (() => {
      if(!isFilterActive) {
        return limit ? allRefrigerators.slice(0, limit) : allRefrigerators;
      }
            ...
            <AnimatePresence mode="wait">
              {loading ? (
                ...
                <>
                  ...
                  >
                    {extraCard}
                    {displayedRefrigerators.map((refrigerators, index) => (
                      <div
                        onClick = {() => navigate(`/refrigerator/details/${refrigerators._id}`)}
                      ...
                      >
                        ...
                        <p className="mt-3 sm:mt-4 text-gray-300 text-sm sm:text-base">
                          {refrigerators.companyName} {refrigerators.name} {refrigerators.colour} ...
                        </p>
                        <p className="mt-2 text-gray-400 text-sm">{refrigerators.price}</p>
                  ...
                  {limit && allRefrigerators.length > limit && (
                    <motion.div
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                    ...
                    >
                      <div className="flex justify-center mt-8">
                        <button
                          onClick={() => navigate("/all/refrigerators")}
