...
function Ac({ limit, extraCard, maxPrice, isFilterActive }) {
  const navigate = useNavigate();
  const [allAcs, setAllAcs] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("https://e-commerce-3-zvas.onrender.com/user/user/get/ac", {
      method: "GET",
      headers: { "Content-type": "application/json" },
    })
      .then((res) => res.json())
      .then((data) => {
        setAllAcs(data.allAcs);
        console.log(allAcs);
  ...
  const displayedAcs = (() => {
    if(!isFilterActive) {
      return limit ? allAcs.slice(0, limit) : allAcs;
    }
    return allAcs
      .filter(item => {
        ...
        <AnimatePresence mode="wait">
          {loading ? (
            ...
            <>
              ...
              >
                {extraCard}
                {displayedAcs.map((ac, index) => (
                  <div
                    onClick = {() => navigate(`/ac/details/${ac._id}`)}
                  ...
                  >
                    ...
                    <p className="mt-3 sm:mt-4 text-gray-300 text-sm sm:text-base">
                      {ac.companyName} {ac.name} {ac.ton} ...
                    </p>
                    <p className="mt-2 text-gray-400 text-sm">{ac.price}</p>
