...
function Laptops({ limit, extraCard, maxPrice, isFilterActive }) {
  const navigate = useNavigate();
  const [allLaptops, setAllLaptops] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("https://e-commerce-3-zvas.onrender.com/user/get/laptops", {
      method: "GET",
      headers: { "Content-type": "application/json" },
    })
      .then((res) => res.json())
      .then((data) => {
        setAllLaptops(data.allLaptops);
  ...
  const displayedLaptops = (() => {
    if(!isFilterActive) {
      return limit ? allLaptops.slice(0, limit) : allLaptops;
    }
    return allLaptops
      .filter(item => {
            ...
            <>
              ...
              >
                ...
                {displayedLaptops.map((laptop, index) => (
                  <div
                    onClick = {() => navigate(`/laptop/details/${laptop._id}`)}
