...
function Jackets({ limit, extraCard, maxPrice, isFilterActive }) {
    const navigate = useNavigate();
    const [allJackets, setAllJackets] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        fetch("https://e-commerce-3-zvas.onrender.com/user/get/jacket", {
            method : "GET",
            headers : { "Content-type" : "application/json" }
        })
          .then((res) => res.json())
          .then((data) => {
            setAllJackets(data.allJackets);
            setTimeout(() => setLoading(false), 1000);
          });
    }, []);

    const displayedJackets = (() => {
      if(!isFilterActive) {
        return limit ? allJackets.slice(0, limit) : allJackets;
      }
      return allJackets
        .filter(item => {
            ...
            <AnimatePresence mode="wait">
              {loading ? (
                ...
                <>
                  ...
                  >
                    {extraCard}
                    {displayedJackets.map((jackets, index) => (
                      <div
                        onClick = {() => navigate(`/jacket/details/${jackets._id}`)}
                      ...
                      >
                        ...
                        <p className="mt-3 sm:mt-4 text-gray-300 text-sm sm:text-base">
                          {jackets.companyName} {jackets.name} {jackets.colour} ...
                        </p>
                        <p className="mt-2 text-gray-400 text-sm">{jackets.price}</p>
