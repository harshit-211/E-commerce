...
function TShirts({ limit, extraCard, maxPrice, isFilterActive }) {
    const navigate = useNavigate();
    const [allTShirts, setAllTShirts] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        fetch("https://e-commerce-3-zvas.onrender.com/user/get/tshirts", {
            method : "GET",
            headers : { 'Content-type' : 'application/json'}
        })
          .then((res) => res.json())
          .then((data) => {
            setAllTShirts(data.allTShirts);
            setTimeout(() => setLoading(false), 1000);
          });
    }, []);

    const displayedTShirts = (() => {
      if(!isFilterActive) {
        return limit ? allTShirts.slice(0, limit) : allTShirts;
      }
      return allTShirts
        .filter(item => {
            ...
            <AnimatePresence mode="wait">
              {loading ? (
                ...
                <>
                  ...
                  >
                    {extraCard}
                    {displayedTShirts.map((tshirts, index) => (
                      <div
                        onClick = {() => navigate(`/tshirt/details/${tshirts._id}`)}
                      ...
                      >
                        ...
                        <p className="mt-3 sm:mt-4 text-gray-300 text-sm sm:text-base">
                          {tshirts.companyName} {tshirts.name} {tshirts.colour}
                        </p>
                        <p className="mt-2 text-gray-400 text-sm">{tshirts.price}</p>
