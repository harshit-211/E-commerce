...
function Watches({ limit, extraCard, maxPrice, isFilterActive }) {
    const navigate = useNavigate();
    const [allWatches, setAllWatches] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        fetch("https://e-commerce-3-zvas.onrender.com/user/get/watches", {
            method : "GET",
            headers : { "Content-type" : "application/json" }
        })
          .then((res) => res.json())
          .then((data) => {
            setAllWatches(data.allWatches);
            setTimeout(() => setLoading(false), 1000);
          });
    }, []);

    const displayedWatches = (() => {
      if(!isFilterActive) {
        return limit ? allWatches.slice(0, limit) : allWatches;
      }
      return allWatches
        .filter(item => {
            ...
            <AnimatePresence mode="wait">
              {loading ? (
                ...
                <>
                  ...
                  >
                    {extraCard}
                    {displayedWatches.map((watches, index) => (
                      <div
                        onClick = {() => navigate(`/watches/details/${watches._id}`)}
                      ...
                      >
                        ...
                        <p className="mt-3 sm:mt-4 text-gray-300 text-sm sm:text-base">
                          {watches.companyName} {watches.name} {watches.colour}
                        </p>
                        <p className="mt-2 text-gray-400 text-sm">{watches.price}</p>
