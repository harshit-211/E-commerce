...
function Tvs({ limit, extraCard, maxPrice, isFilterActive }) {
    const navigate = useNavigate();
    const [allTv, setAllTv] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        fetch("https://e-commerce-3-zvas.onrender.com/user/get/tvs", {
            method : "GET",
    ...
    const displayedTv = (() => {
      if(!isFilterActive) {
        return limit ? allTv.slice(0, limit) : allTv;
      }
      return allTv
        .filter(item => {
            ...
            <AnimatePresence mode="wait">
              {loading ? (
                ...
                <>
                  ...
                  >
                    {extraCard}
                    {displayedTv.map((tvs, index) => (
                      <div
                        onClick = {() => navigate(`/tv/details/${tvs._id}`)}
                      ...
                      >
                        ...
                        <p className="mt-3 sm:mt-4 text-gray-300 text-sm sm:text-base">
                          {tvs.companyName} {tvs.name} {tvs.colour}
                        </p>
                        <p className="mt-2 text-gray-400 text-sm">{tvs.price}</p>
                        ...
                        <button 
                          ...
                          className="bg-cyan-400 w-full rounded-md shadow mt-4 h-10 flex items-center justify-center gap-2 text-gray-900 font-semibold text-sm transition-all duration-200 transform hover:scale-105 hover:bg-cyan-300 hover:shadow-cyan-400/50 hover:shadow-lg hover:cusor-pointer"
                        >
