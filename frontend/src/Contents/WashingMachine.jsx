...
function WashingMachine({ limit, extraCard, maxPrice, isFilterActive }) {
    const navigate = useNavigate();
    const [allWashingMachine, setAllWashingMachine] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        fetch("https://e-commerce-3-zvas.onrender.com/user/get/washingMachine", {
            method : "GET",
            headers : { 'Content-type' : 'application/json' }
        })
          .then((res) => res.json())
          .then((data) => {
            setAllWashingMachine(data.allWashingMachines);
            setTimeout(() => setLoading(false), 1000);
          });
    }, []);

    const displayedWashingMachine = (() => {
      if(!isFilterActive) {
        return limit ? allWashingMachine.slice(0, limit) : allWashingMachine;
      }
      return allWashingMachine
        .filter(item => {
            ...
            <AnimatePresence mode="wait">
              {loading ? (
                ...
                <>
                  ...
                  >
                    {extraCard}
                    {displayedWashingMachine.map((washingMachine, index) => (
                      <div
                        onClick = {() => navigate(`/washingMachine/details/${washingMachine._id}`)}
                      ...
                      >
                        ...
                        <p className="mt-3 sm:mt-4 text-gray-300 text-sm sm:text-base">
                          {washingMachine.companyName} {washingMachine.name} {washingMachine.colour}
                        </p>
                        <p className="mt-2 text-gray-400 text-sm">{washingMachine.price}</p>
                        ...
                        <button 
                          onClick = {(e) => {
                            ...
                            fetch("https://e-commerce-3-zvas.onrender.com/add/to/cart", {
                              ...
                              body : JSON.stringify({
                                productId : washingMachine._id,
                                productType : "WashingMachine"
                              })
                            })
                              .then((res) => res.json())
                              .then((data) => {
                  ...
                  {limit && allWashingMachine.length > limit && (
                    <motion.div
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                    ...
                    >
                      <div className="flex justify-center mt-8">
                        <button
                          onClick={() => navigate("/all/washingMachines")}
