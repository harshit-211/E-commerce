...
function Shirts({ limit, extraCard, maxPrice, isFilterActive }) {
    const navigate = useNavigate();
    const [allShirts, setAllShirts] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        fetch("https://e-commerce-3-zvas.onrender.com/user/get/shirts", {
            method : "GET",
            headers : { "Content-type" : "application/json" }
        })
          .then((res) => res.json())
          .then((data) => {
            setAllShirts(data.allShirts);
            setTimeout(() => setLoading(false), 1000);
          });
    }, []);

    const displayedShirts = (() => {
      if(!isFilterActive) {
        return limit ? allShirts.slice(0, limit) : allShirts;
      }
      return allShirts
        .filter(item => {
            ...
            <AnimatePresence mode="wait">
              {loading ? (
                ...
                <>
                  ...
                  >
                    {extraCard}
                    {displayedShirts.map((shirts, index) => (
                      <div
                        onClick = {() => navigate(`/shirt/details/${shirts._id}`)}
                      ...
                      >
                        ...
                        <p className="mt-3 sm:mt-4 text-gray-300 text-sm sm:text-base">
                          {shirts.companyName} {shirts.name} {shirts.colour} ...
                        </p>
                        <p className="mt-2 text-gray-400 text-sm">{shirts.price}</p>
