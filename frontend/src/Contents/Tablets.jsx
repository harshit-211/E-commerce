...
function Tablets({ limit, extraCard, maxPrice, isFilterActive }) {
    const navigate = useNavigate();
    const [allTablets, setAllTablets] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        fetch("https://e-commerce-3-zvas.onrender.com/user/get/tablets", {
            method : "GET",
            headers : { "Content-type" : "application/json" }
        })
          .then((res) => res.json())
          .then((data) => {
            setAllTablets(data.allTablets);
            setTimeout(() => setLoading(false), 1000);
          });
    }, []);

    const displayedTablets = (() => {
      if(!isFilterActive) {
        return limit ? allTablets.slice(0, limit) : allTablets;
      }
      return allTablets
        .filter(item => {
            ...
            <AnimatePresence mode="wait">
              {loading ? (
                ...
                <>
                  ...
                  >
                    {extraCard}
                    {displayedTablets.map((tablets, index) => (
                      <div
                        onClick = {() => navigate(`/tablet/details/${tablets._id}`)}
                      ...
                      >
                        ...
                        <p className="mt-3 sm:mt-4 text-gray-300 text-sm sm:text-base">
                          {tablets.companyName} {tablets.name} {tablets.colour}
                        </p>
                        <p className="mt-2 text-gray-400 text-sm">{tablets.price}</p>
