...
function Jeans({ limit, extraCard, maxPrice, isFilterActive }) {
    const navigate = useNavigate();
    const [allJeans, setAllJeans] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        fetch("https://e-commerce-3-zvas.onrender.com/user/get/jeans", {
            method : "GET",
            headers : { "Content-type" : "application/json" }
        })
          .then((res) => res.json())
          .then((data) => {
            setAllJeans(data.allJeans);
            setTimeout(() => setLoading(false), 1000);
          });
    }, []);

    const displayedJeans = (() => {
      if(!isFilterActive) {
        return limit ? allJeans.slice(0, limit) : allJeans;
      }
      return allJeans
        .filter(item => {
            ...
            <AnimatePresence mode="wait">
              {loading ? (
                ...
                <>
                  ...
                  >
                    {extraCard}
                    {displayedJeans.map((jeans, index) => (
                      <div
                        onClick = {() => navigate(`/jean/details/${jeans._id}`)}
                      ...
                      >
                        ...
                        <p className="mt-3 sm:mt-4 text-gray-300 text-sm sm:text-base">
                          {jeans.companyName} {jeans.name} {jeans.colour} ...
                        </p>
                        <p className="mt-2 text-gray-400 text-sm">{jeans.price}</p>
