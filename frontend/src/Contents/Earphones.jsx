...
function Earphones({ limit, extraCard, maxPrice, isFilterActive }) {
    const navigate = useNavigate();
    const [allEarphones, setAllEarphones] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        fetch("https://e-commerce-3-zvas.onrender.com/user/get/earphones", {
            method : "GET",
            headers : { "Content-type" : "application/json" }
        })
          .then((res) => res.json())
          .then((data) => {
            setAllEarphones(data.allEarphones);
            setTimeout(() => setLoading(false), 1000);
          });
    }, []);

    const displayedEarphones = (() => {
      if(!isFilterActive) {
        return limit ? allEarphones.slice(0, limit) : allEarphones;
      }
      return allEarphones
        .filter(item => {
            ...
            <AnimatePresence mode="wait">
              {loading ? (
                ...
                <>
                  ...
                  >
                    {extraCard}
                    {displayedEarphones.map((earphones, index) => (
                      <div
                        onClick = {() => navigate(`/earphone/details/${earphones._id}`)}
                      ...
                      >
                        ...
                        <p className="mt-3 sm:mt-4 text-gray-300 text-sm sm:text-base">
                          {earphones.companyName} {earphones.name} {earphones.batteryLife}
                        </p>
                        <p className="mt-2 text-gray-400 text-sm">{earphones.price}</p>
                  ...
                  {limit && allEarphones.length > limit && (
                    <motion.div
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                    ...
                    >
                      <div className="flex justify-center mt-8">
                        <button
                          onClick={() => navigate("/all/earphones")}
