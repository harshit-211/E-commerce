...
function RefrigeratorsDetails() {
    const { id } = useParams();
    const [item, setItem] = useState(null);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        fetch(`https://e-commerce-3-zvas.onrender.com/user/get/refrigerator/${id}`, {
            method : "GET",
          ...
          <div className = "flex flex-col md:flex-row items-center justify-center gap-16 mt-16">
            ...
            <div className = "max-w-lg text-center md:text-left">
              ...
              <p className = "mt-4 text-gray-300 text-base md:text-lg leading-relaxed">
                {item.size} | {item.star} | {item.colour} colour | {item.quantity} left only
              </p>
...
export default RefrigeratorsDetails;