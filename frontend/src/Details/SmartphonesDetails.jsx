...
function SmartphonesDetails() {
    ...
    if(!item) {
        return <div className = "text-center mt-10 text-gray-400">
            Item not found.
        </div>
    }
          ...
          <div className = "flex flex-col md:flex-row items-center justify-center gap-16 mt-16">
            ...
            <div className = "max-w-lg text-center md:text-left">
              ...
              <p className = "mt-4 text-gray-300 text-base md:text-lg leading-relaxed">
                {item.processor} | {item.ram} RAM | {item.storage} Storage | {item.camera} | {item.colour} colour | 
                {item.size} | {item.batteryCapacity} | {item.operatingSystem} | {item.quantity} left only
              </p>
