...
function LaptopDetails() {
    ...
    useEffect(() => {
        fetch(`https://e-commerce-3-zvas.onrender.com/user/get/laptops/${id}`, {
            method : "GET",
          ...
          <div className = "flex flex-col md:flex-row items-center justify-center gap-16 mt-16">
            ...
            <div className = "max-w-lg text-center md:text-left">
              ...
              <p className = "mt-4 text-gray-300 text-base md:text-lg leading-relaxed">
                {item.processor} | {item.graphicsCard} | {item.ram} RAM | {item.storage} Storage | {item.colour} colour 
                | {item.screenSize} | {item.batteryCapacity} | {item.operatingSystem} | {item.weight} | {item.quantity} left only
              </p>
